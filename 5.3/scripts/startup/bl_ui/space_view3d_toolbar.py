...
class VIEW3D_PT_tools_meshedit_options_mirror(View3DPanel, Panel):
    """Top-bar mesh options popover (same place as the topology-mirror add-on)."""
    ...
    def draw(self, context):
        ...
        col = layout.column(align=True)
        mirrored = mesh.use_mirror_x or mesh.use_mirror_y or mesh.use_mirror_z
        col.active = mirrored
        col.prop(overlay, "show_mirror_highlight", text="Highlight Counterparts")
        sub = col.row(align=True)
        sub.active = mirrored and overlay.show_mirror_highlight
        sub.prop(overlay, "mirror_highlight_color", text="Color")
        if not mesh.use_mirror_topology:
            col.prop(tool_settings, "mesh_mirror_threshold", text="Pair Distance")
        if not mirrored:
            layout.label(text="No mirror axis is enabled", icon='INFO')
...
classes = (
    ...
    VIEW3D_PT_tools_meshedit_options_mirror,
