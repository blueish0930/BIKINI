...
class VIEW3D_PT_overlay_viewer_node(Panel):
    ...
    def draw(self, context):
        ...
        row.prop(overlay, "show_viewer_text", text="")
        subrow = row.row(align=True)
        subrow.active = overlay.show_viewer_text
        subrow.prop(overlay, "viewer_attribute_text_size", text="Attribute Text")

        row = col.row(align=True)
        row.active = view.show_viewer and overlay.show_text and overlay.show_viewer_text
        row.prop(
            overlay,
            "use_viewer_attribute_text_backface_culling",
